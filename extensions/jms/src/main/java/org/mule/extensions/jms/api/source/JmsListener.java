...
public class JmsListener extends Source<Object, JmsAttributes> {
  ...
  public void onStart(SourceCallback<Object, JmsAttributes> sourceCallback) throws MuleException {
    ...
    try {
      ...
      consumer.listen(message -> {
        ...
        if (message != null) {
          //TODO change NONE ACK to end of flow (Creo no tiene sentido)
          JmsListener.this.evaluateAckAction(sourceCallback, session, message, jmsLock);
          encoding = JmsListener.this.resolveEncoding(message);
          contentType = JmsListener.this.resolveContentType(message);
          JmsListener.this.saveReplyToDestination(sourceCallback, message, context);
        }

        JmsListener.this.produceMessageResult(sourceCallback, jmsSupport, session, message, context);
  ...
  public void onError(Error error) {
    try {
      LOGGER.error("RECOVERING SESSION");
    } finally {
  ...
  private void produceMessageResult(SourceCallback<Object, JmsAttributes> sourceCallback, JmsSupport jmsSupport,
    ...
    try {
      ...
      Result<Object, JmsAttributes> result = resultFactory.createResult(message, jmsSupport.getSpecification(), contentType,
                                                                        encoding, config.getDefaultEncoding(), session.getAckId());
