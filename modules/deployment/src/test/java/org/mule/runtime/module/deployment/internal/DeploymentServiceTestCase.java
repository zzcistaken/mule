...
public class DeploymentServiceTestCase extends AbstractMuleTestCase {
  ...
  public void setUp() throws Exception {
    ...
    final File domainFolder = getDomainFolder(Domain.DEFAULT_DOMAIN_NAME);
    ...
    applicationDeploymentListener = Mockito.mock(DeploymentListener.class);
    domainDeploymentListener = Mockito.mock(DeploymentListener.class);
  ...
  public void deploysBrokenAppZipOnStartup() throws Exception {
    ...
    Assert.assertEquals("Wrong number of zombie apps registered.", 1, zombieMap.size());
    final Map.Entry<URL, Long> zombie = zombieMap.entrySet().iterator().next();
    Assert.assertEquals("Wrong URL tagged as zombie.", brokenAppFileBuilder.getDeployedPath(),
                        new File(zombie.getKey().getFile()).getName());
    Assert.assertTrue("Invalid lastModified value for file URL.", zombie.getValue() != -1);
  }
  ...
  public void doesNotRetriesBrokenAppWithFunkyName() throws Exception {
    ...
    Assert.assertEquals("Wrong number of zombie apps registered.", 1, zombieMap.size());
    final Map.Entry<URL, Long> zombie = zombieMap.entrySet().iterator().next();
    Assert.assertEquals("Wrong URL tagged as zombie.", brokenAppWithFunkyNameAppFileBuilder.getDeployedPath(),
                        new File(zombie.getKey().getFile()).getName());
    Assert.assertTrue("Invalid lastModified value for file URL.", zombie.getValue() != -1);

    Mockito.reset(applicationDeploymentListener);

    addPackedAppFromBuilder(dummyAppDescriptorFileBuilder);

    assertApplicationDeploymentSuccess(applicationDeploymentListener, dummyAppDescriptorFileBuilder.getId());
    assertDeploymentFailure(applicationDeploymentListener, brokenAppWithFunkyNameAppFileBuilder.getId(), Mockito.never());
  ...
  public void deploysBrokenAppZipAfterStartup() throws Exception {
    ...
    Assert.assertEquals("Wrong number of zombie apps registered.", 1, zombieMap.size());
    final Map.Entry<URL, Long> zombie = zombieMap.entrySet().iterator().next();
    Assert.assertEquals("Wrong URL tagged as zombie.", "broken-app.zip", new File(zombie.getKey().getFile()).getName());
    Assert.assertTrue("Invalid lastModified value for file URL.", zombie.getValue() != -1);
  }
  ...
  public void deploysInvalidExplodedAppOnStartup() throws Exception {
    ...
    Assert.assertEquals("Wrong URL tagged as zombie.", "app with spaces", appName);
    Assert.assertTrue("Invalid lastModified value for file URL.", zombie.getValue() != -1);
  }
  ...
  public void deploysInvalidExplodedOnlyOnce() throws Exception {
    ...
    addExplodedAppFromBuilder(emptyAppFileBuilder, "app with spaces");
    assertDeploymentFailure(applicationDeploymentListener, "app with spaces", Mockito.times(1));
  ...
  public void deploysBrokenExplodedAppOnStartup() throws Exception {
    ...
    Assert.assertEquals("Wrong number of zombie apps registered.", 1, zombieMap.size());
    final Map.Entry<URL, Long> zombie = zombieMap.entrySet().iterator().next();
    Assert.assertEquals("Wrong URL tagged as zombie.", incompleteAppFileBuilder.getId(),
  ...
  public void deploysInvalidZipAppOnStartup() throws Exception {
    ...
    // Spaces are converted to %20 is returned by java file api :/
    String appName = URLDecoder.decode(new File(zombie.getKey().getFile()).getName(), "UTF-8");
    Assert.assertEquals("Wrong URL tagged as zombie.", "app with spaces.zip", appName);
    Assert.assertTrue("Invalid lastModified value for file URL.", zombie.getValue() != -1);
  }
  ...
  public void deploysAppZipWithExtensionPlugin() throws Exception {
    ...
    org.apache.commons.io.FileUtils.copyFile(echoService.getArtifactFile(), installedService);
    ...
    org.apache.commons.io.FileUtils.copyFile(fooService.getArtifactFile(), installedService);
  ...
  public void deploysDomainZipOnStartup() throws Exception {
    ...
    assertDomainDir(NONE, new String[] {Domain.DEFAULT_DOMAIN_NAME, emptyDomainFileBuilder.getId()}, true);
  ...
  private void deploysDomainBundle() {
    ...
    assertDomainDir(NONE, new String[] {Domain.DEFAULT_DOMAIN_NAME, dummyDomainBundleFileBuilder.getId()}, true);
  ...
  private void deploysInvalidDomainBundleZip() {
    ...
    assertDomainDir(NONE, new String[] {Domain.DEFAULT_DOMAIN_NAME, invalidDomainBundleFileBuilder.getId()}, true);
  ...
  public void deploysBrokenDomainZipOnStartup() throws Exception {
    ...
    assertDomainDir(new String[] {brokenDomainFileBuilder.getDeployedPath()}, new String[] {Domain.DEFAULT_DOMAIN_NAME}, true);
  ...
  public void redeploysDomainZipDeployedOnStartup() throws Exception {
    ...
    assertDomainDir(NONE, new String[] {Domain.DEFAULT_DOMAIN_NAME, emptyAppFileBuilder.getId()}, true);
  ...
  public void redeploysDomainZipDeployedAfterStartup() throws Exception {
    ...
    assertDomainDir(NONE, new String[] {Domain.DEFAULT_DOMAIN_NAME, dummyDomainFileBuilder.getId()}, true);
    assertEquals("Domain has not been properly registered with Mule", 2, deploymentService.getDomains().size());
  ...
  public void deploysInvalidExplodedDomainOnStartup() throws Exception {
    ...
    // Maintains app dir created
    assertDomainDir(NONE, new String[] {Domain.DEFAULT_DOMAIN_NAME, "domain with spaces"}, true);
    final Map<URL, Long> zombieMap = deploymentService.getZombieDomains();
    assertEquals("Wrong number of zombie domains registered.", 1, zombieMap.size());
    final Map.Entry<URL, Long> zombie = zombieMap.entrySet().iterator().next();
    // Spaces are converted to %20 is returned by java file api :/
  ...
  public void deploysBrokenExplodedDomainOnStartup() throws Exception {
    ...
    // Maintains app dir created
    assertDomainDir(NONE, new String[] {Domain.DEFAULT_DOMAIN_NAME, incompleteDomainFileBuilder.getId()}, true);
    final Map<URL, Long> zombieMap = deploymentService.getZombieDomains();
    assertEquals("Wrong number of zombie domains registered.", 1, zombieMap.size());
    final Map.Entry<URL, Long> zombie = zombieMap.entrySet().iterator().next();
  ...
  private DefaultMuleDomain createDefaultDomain() {
    ...
    descriptor.setName(Domain.DEFAULT_DOMAIN_NAME);
