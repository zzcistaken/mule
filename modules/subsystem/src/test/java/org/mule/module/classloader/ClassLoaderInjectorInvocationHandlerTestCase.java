...
public class ClassLoaderInjectorInvocationHandlerTestCase extends AbstractMuleTestCase
{
    ...
    {
        final ClassLoader classLoader = new ModuleClassLoader(Thread.currentThread().getContextClassLoader(), new URL[0], null);
        TestDelegate delegate = mock(TestDelegate.class);
        doAnswer(new Answer()
        {
            @Override
    ...
    public void restoresOriginalClassLoaderAfterMethodDelegation() throws Exception
    {
        ...
        TestDelegate delegate = mock(TestDelegate.class);
        ClassLoader classLoader = new ModuleClassLoader(Thread.currentThread().getContextClassLoader(), new URL[0], null);
