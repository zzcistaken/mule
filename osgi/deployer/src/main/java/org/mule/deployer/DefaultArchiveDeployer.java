 ...
 */
public class DefaultArchiveDeployer<T extends ArtifactBundle> implements ArchiveDeployer<T>
{
    ...
    private final ArtifactBundleDeployer<T> deployer;
    ...
    private final ArtifactBundleFactory<T> bundleFactory;
    private DeploymentListener deploymentListener = new NullDeploymentListener();


    public DefaultArchiveDeployer(final ArtifactBundleDeployer deployer, final ObservableList<T> artifacts, final ReentrantLock lock,
                                  ArtifactDeploymentTemplate deploymentTemplate, ArtifactBundleFactory<T> bundleFactory)
    {
        ...
        this.artifactDir = bundleFactory.getArtifactDir();
    ...
    {
        //TODO(pablo.kraan): try to remove this method
        return artifactDir;
    }
    ...
    {
        final T artifactBundle;
        try
        {
            try
            {
                artifactBundle = guardedInstallFrom(artifactAchivedUrl);
                trackArtifact(artifactBundle);
            }
            catch (Throwable t)
            {
    ...
    ArtifactBundleDeployer getDeployer()
    {
        return deployer;
    }
    ...
    private T deployPackagedArtifact(final URL artifactUrl, String artifactName) throws IOException
    {
        ...
        //TODO(pablo.kraan): OSGi - add redeployment
        ...
        T deployedArtifact = deployPackagedArtifact(artifactUrl);
    ...
    private T deployExplodedApp(String addedApp) throws DeploymentException
    {
        ...
        try
        {
            artifact = bundleFactory.create(addedApp);

            if (artifactArchiveInstaller.getArtifactManifest(addedApp) == null)
            {
                artifactArchiveInstaller.createArtifactManifest(addedApp);
            }
    ...
    private void guardedDeploy(T artifactBundle)
    {
        try
        {
            if (!deploymentLock.tryLock(0, TimeUnit.SECONDS))
            ...
            deployer.deploy(artifactBundle);
        }
        catch (InterruptedException e)
        {
            Thread.currentThread().interrupt();
    ...
    @Override
    public void deployArtifact(T artifactBundle) throws DeploymentException
    {
        try
        {
            deploymentListener.onDeploymentStart(artifactBundle.getArtifactName());
            guardedDeploy(artifactBundle);
            artifactArchiveInstaller.createAnchorFile(artifactBundle.getArtifactName());
            deploymentListener.onDeploymentSuccess(artifactBundle.getArtifactName());
            artifactZombieMap.remove(artifactBundle.getArtifactName());
        }
        catch (Throwable t)
        {
            // error text has been created by the deployer already
            String msg = miniSplash(String.format("Failed to deploy artifact '%s', see below", artifactBundle.getArtifactName()));
            logger.error(msg, t);

            addZombieApp(artifactBundle);

            deploymentListener.onDeploymentFailure(artifactBundle.getArtifactName(), t);
            if (t instanceof DeploymentException)
            {
                throw (DeploymentException) t;
            }
            ...
            {
                msg = "Failed to deploy artifact: " + artifactBundle.getArtifactName();
                throw new DeploymentException(MessageFactory.createStaticMessage(msg), t);
            }
        }
    }

    private void addZombieApp(ArtifactBundle artifact)
    {
    ...
    private T installFrom(URL url) throws IOException
    {
        ...
        return bundleFactory.create(artifactName);
    }
