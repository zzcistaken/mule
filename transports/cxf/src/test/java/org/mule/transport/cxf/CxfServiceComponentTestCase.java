...
public class CxfServiceComponentTestCase extends AbstractMuleTestCase
{
    ...
    protected void doSetUp() throws Exception
    {
        ...
        receiver.connector = new CxfConnector();
        receiver.connector.setCxfBus(cxfBus);
        final CxfConnector connector = new CxfConnector();
    ...
    private OutputHandler executeAndValidateCallToSentToDestination() throws MuleException, IOException
    {
        ...
        final MuleMessage muleReqMsg = new DefaultMuleMessage("some object");
        ...
        assertTrue(muleResMsg.getAdapter() instanceof DefaultMessageAdapter);

        return extractOutputHandlerFromDefaultMuleMessage(muleResMsg);
    }

    private OutputHandler extractOutputHandlerFromDefaultMuleMessage(DefaultMuleMessage muleResMsg)
    {
        DefaultMessageAdapter adapter = (DefaultMessageAdapter) muleResMsg.getAdapter();
        assertTrue(adapter.getPayload() instanceof OutputHandler);
        OutputHandler outputHandler = (OutputHandler) adapter.getPayload();
